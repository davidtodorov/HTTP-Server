...
namespace SIS.HTTP.Requests
{
    public class HttpRequest : IHttpRequest
    {
        ...
        {
            var plainHeadersCollection = plainHeaders.Select(ph => ph
                .Split(new[] { ':', ' ' }, StringSplitOptions.RemoveEmptyEntries))
                .ToList();

            foreach (var phKvp in plainHeadersCollection)
            {
                this.Headers.AddHeader(new HttpHeader(phKvp[0], phKvp[1]));
            }
        }

        private IEnumerable<string> ParsePlainRequestHeaders(string[] requestLineParams)
        {
            for (int i = 1; i < requestLineParams.Length - 1; i++)
            {
                if (!string.IsNullOrEmpty(requestLineParams[i]))
                {
                    yield return requestLineParams[i];
                }
            }
        }
        ...
        private void AddQueryParametersToDictionary(string[] queryKvp, Dictionary<string, object> dictWithParams)
        {
            ...
            {
                dictWithParams.Add(queryKvp[0], queryKvp[1]);
            }
        }
